...
const CategoryForm = ({
  ...
  const handleSubmit = async (data: FormData) => {
    ...
    const pendingCategory: Category = {
      updatedAt: category?.updatedAt ?? new Date().toISOString().slice(0, 19).replace("T", " "),
      createdAt: category?.createdAt ?? new Date().toISOString().slice(0, 19).replace("T", " "),
      id: category?.id ?? "",
      ...values,
    };
    try {
    ...
    <form action={handleSubmit} onChange={handleChange} className={"space-y-8"}>
      {/* Schema fields start */}
        ...
        <Label
          className={cn(
            "mb-2 inline-block",
            errors?.categoryName ? "text-destructive" : "",
          )}
        >
          Category Name
        </Label>
        <Input
          type="text"
          name="categoryName"
          className={cn(errors?.categoryName ? "ring ring-destructive" : "")}
          defaultValue={category?.categoryName ?? ""}
        />
        {errors?.categoryName ? (
          <p className="text-xs text-destructive mt-2">{errors.categoryName[0]}</p>
        ) : (
          <div className="h-6" />
        )}
      </div>
